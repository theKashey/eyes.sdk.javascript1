...
const VG_BACKWARD_COMPATIBLE = ['Samsung Galaxy S8', 'Samsung Galaxy A5', 'Galaxy S III', 'Galaxy Note II']
...
describe('DeviceName', function() {
  ...
  before(async () => {
    ...
    expectdDeviceNames = expectdDeviceNames.filter(device => !VG_BACKWARD_COMPATIBLE.includes(device))
  })

  it('has correct values', async function() {
    expectdDeviceNames = assert.deepStrictEqual(Object.values(DeviceName).sort(), expectdDeviceNames.sort())
  })
})
