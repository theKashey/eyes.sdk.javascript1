  ...
  it('works for openBeforeVisit.js', async () => {
    try {
      await pexec(
        ...
        {
          maxBuffer: 10000000,
          // in case we don't call runningTests.abortTests() from handlers, then the test will fail after 30 seconds, but Cypress will continue to run in the background which is undesired
          timeout: 30000,
        },
      );
    } catch (ex) {
