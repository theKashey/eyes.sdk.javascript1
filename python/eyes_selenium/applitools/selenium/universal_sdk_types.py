...
@attr.s
class CodedRegionReference(object):
...
def padded_region_references_convert(padded_regions):
    # type: (List[GetRegion]) -> List[CodedRegionReference]
    results = []
    for r in padded_regions:
        if isinstance(r, RegionBySelector):
            padded_region = CodedRegionReference(
            ...
            results.append(CodedRegionReference(Region.convert(r._region)))  # noqa
        else:
            raise RuntimeError("Unexpected region type", type(r))
    return results
...
class MatchSettings(object):
    ...
    ignore_regions = attr.ib(default=None)  # type: Optional[List[CodedRegionReference]]
    layout_regions = attr.ib(default=None)  # type: Optional[List[CodedRegionReference]]
    strict_regions = attr.ib(default=None)  # type: Optional[List[CodedRegionReference]]
    content_regions = attr.ib(
        default=None
    )  # type: Optional[List[CodedRegionReference]]
