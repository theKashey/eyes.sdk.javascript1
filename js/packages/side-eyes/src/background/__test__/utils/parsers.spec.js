...
describe('parsers', () => {
  ...
  describe('parseBrowsers', () => {
    ...
    it('parses browsers and viewports', () => {
      const result = parseBrowsers(['Firefox', 'IE10'], ['800x600', '1024x768']).matrix
    ...
    it.skip('parses experimental browsers', () => {
      const { matrix, didRemoveResolution } = parseBrowsers(['Edge'], ['800x600', '1324x768'])
    ...
    it('parses devices and orientations for chrome emulation', () => {
      const result = parseBrowsers([], [], [{ name: 'iPhone 4', type: 'emulator' }], ['Landscape', 'Portrait']).matrix
    ...
    it('parses devices and orientations for ios simulators', () => {
      const result = parseBrowsers([], [], [{ name: 'iPhone 11', type: 'simulator' }], ['Landscape', 'Portrait']).matrix
      expect(result[0].iosDeviceInfo.deviceName).toEqual('iPhone 11')
      expect(result[0].iosDeviceInfo.screenOrientation).toEqual('landscapeLeft')
      expect(result[1].iosDeviceInfo.deviceName).toEqual('iPhone 11')
      expect(result[1].iosDeviceInfo.screenOrientation).toEqual('portrait')
    })
