...
const {takeSnapshot} = require('@applitools/nml-client')
...
async function takeVHSes({driver, browsers, apiKey, serverUrl, proxy, waitBeforeCapture, logger}) {
  ...
  if (brokerURL) {
    ...
    const nmlSnapshot = await takeSnapshot(brokerURL, {proxy: proxy && proxy.toReqObject(), logger})
    const snapshot = {
    ...
    log('taking NMG snapshot with the legacy protocol')
