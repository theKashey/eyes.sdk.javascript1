...
describe('transform-target', () => {
  ...
  describe('formats', () => {
    ...
    it('image buffer in png format', async () => {
      const result = await transformTarget({target: {image: readFileSync('./test/fixtures/screenshot.png')}, settings: {}})
    ...
    it('image path in png format', async () => {
      const result = await transformTarget({target: {image: resolvePath('./test/fixtures/screenshot.png')}, settings: {}})
      assert(Buffer.compare(result.image as Buffer, expected.png) === 0)
      assert.strict.deepEqual(result.size, {width: 1079, height: 3415})
    })
    ...
    it('image http url object in png format', async () => {
      const result = await transformTarget({target: {image: new URL(`${baseUrl}/screenshot.png`)}, settings: {}})
      assert(Buffer.compare(result.image as Buffer, expected.png) === 0)
      assert.strict.deepEqual(result.size, {width: 1079, height: 3415})
    })

    it('image http url string in png format', async () => {
      const result = await transformTarget({target: {image: `${baseUrl}/screenshot.png`}, settings: {}})
      assert(Buffer.compare(result.image as Buffer, expected.png) === 0)
      assert.strict.deepEqual(result.size, {width: 1079, height: 3415})
    })

    it('image buffer in jpeg format', async () => {
      const result = await transformTarget({target: {image: readFileSync('./test/fixtures/screenshot.jpeg')}, settings: {}})
    ...
    it('image path in jpeg format', async () => {
      const result = await transformTarget({target: {image: resolvePath('./test/fixtures/screenshot.jpeg')}, settings: {}})
      assert(Buffer.compare(result.image as Buffer, expected.jpeg) === 0)
      assert.strict.deepEqual(result.size, {width: 1079, height: 3415})
    })
    ...
    it('image http url object in jpeg format', async () => {
      const result = await transformTarget({target: {image: new URL(`${baseUrl}/screenshot.jpeg`)}, settings: {}})
      assert(Buffer.compare(result.image as Buffer, expected.jpeg) === 0)
      assert.strict.deepEqual(result.size, {width: 1079, height: 3415})
    })

    it('image http url string in jpeg format', async () => {
      const result = await transformTarget({target: {image: `${baseUrl}/screenshot.jpeg`}, settings: {}})
      assert(Buffer.compare(result.image as Buffer, expected.jpeg) === 0)
      assert.strict.deepEqual(result.size, {width: 1079, height: 3415})
    })
  })
