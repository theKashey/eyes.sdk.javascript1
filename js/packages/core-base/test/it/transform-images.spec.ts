...
describe('transform-images', () => {
  describe('transformations', () => {
    it('image normalization', async () => {
      ...
      assert(Buffer.compare(result, expected) === 0)
    })
  ...
  describe('formats', () => {
    ...
    it('image base64 in png format', async () => {
      const result = await transformImage({
        image: readFileSync('./test/fixtures/screenshot.png').toString('base64'),
        settings: {},
      })
      assert(Buffer.compare(result, expected.png) === 0)
    })
    ...
    it('image file url string in png format', async () => {
      const result = await transformImage({
        image: pathToFileURL(resolvePath('./test/fixtures/screenshot.png')).href,
        settings: {},
      })
      assert(Buffer.compare(result, expected.png) === 0)
    })
    ...
    it('image base64 in jpeg format', async () => {
      const result = await transformImage({
        image: readFileSync('./test/fixtures/screenshot.jpeg').toString('base64'),
        settings: {},
      })
      assert(Buffer.compare(result, expected.jpeg) === 0)
    })
    ...
    it('image file url string in jpeg format', async () => {
      const result = await transformImage({
        image: pathToFileURL(resolvePath('./test/fixtures/screenshot.jpeg')).href,
        settings: {},
      })
      assert(Buffer.compare(result, expected.jpeg) === 0)
    })
